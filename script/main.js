...
function setRobbyLeftAndRightEdge() {
    robbyRightEdge = .5 * (containerDiv.offsetWidth + robbyDiv.offsetWidth) - 65,
    robbyLeftEdge = .5 * (containerDiv.offsetWidth - robbyDiv.offsetWidth) + 65
}
...
function shiftUpDownHorizontalLayers() {
    (previousPageVerticalPosition < sea1Div.offsetLeft - robbyLeftEdge || previousPageVerticalPosition > sea1Div.offsetLeft + sea1Div.offsetWidth - robbyRightEdge) && pageVerticalPosition >= sea1Div.offsetLeft - robbyLeftEdge && pageVerticalPosition <= sea1Div.offsetLeft + sea1Div.offsetWidth - robbyRightEdge && (isRobbySwimming = !0,
    shiftUpLayerHorizontal(),
    shiftRobbyToSeaFloor(),
    createBubble()),
    previousPageVerticalPosition >= sea1Div.offsetLeft - robbyLeftEdge && previousPageVerticalPosition <= sea1Div.offsetLeft + sea1Div.offsetWidth - robbyRightEdge && (pageVerticalPosition < sea1Div.offsetLeft - robbyLeftEdge || pageVerticalPosition > sea1Div.offsetLeft + sea1Div.offsetWidth - robbyRightEdge) && (isRobbySwimming = !1,
...
function shiftUpDownHorizontalLayersOnResize() {
    pageVerticalPosition >= sea1Div.offsetLeft - robbyLeftEdge && pageVerticalPosition <= sea1Div.offsetLeft + sea1Div.offsetWidth - robbyRightEdge && (clearInterval(shiftUpLayerHorizontalTimer),
...
function blinkRobbyEyes() {
    "not moving 2" != layersMovement && ($(robbyEyesCloseDiv).fadeTo(0, 1),
...
function positionBalloonAndRobbyContainerHorizontally() {
    ...
    i <= n && (n = i),
    "vertical" == layersMovement ? (balloonDiv.style.left = n + "px",
    robbyContainerDiv.style.left = t + "px") : "not moving 1" == layersMovement || "not moving 2" == layersMovement ? (robbyContainerDiv.style.left = t + pageVerticalPosition - (pageDiv.offsetHeight - containerDiv.offsetHeight - distanceBetweenRobbyAndBalloon) + "px",
    balloonDiv.style.left = n + "px") : (balloonDiv.style.left = layerHorizontalArray[layerHorizontalArray.length - 1].offsetLeft + layerHorizontalArray[layerHorizontalArray.length - 1].offsetWidth - .5 * (containerDiv.offsetWidth + balloonDiv.offsetWidth) + "px",
...
function orientRobby() {
    0 < deltaPageVerticalPosition && (robbyFramesDiv.style.top = "0px",
    robbyEyesCloseDiv.style.left = "33px"),
    deltaPageVerticalPosition < 0 && (robbyFramesDiv.style.top = "-200px",
    robbyEyesCloseDiv.style.left = "5px")
}
function storeDivs() {
...
function animateBuildingsLegs() {
    ...
    for (var e = 0; e < buildingArray.length; e++) {
        if (buildingArray[buildingArray.length - 1].offsetLeft == buildingTargetLeftArray[buildingTargetLeftArray.length - 1])
            return buildingLegFrameArray[buildingLegFrameArray.length - 1].style.left = "0px",
            clearInterval(buildingLegsTimer),
...
function blinkNbaPlayer() {
    pageVerticalPosition + .5 * containerDiv.offsetWidth < about3ContainerDiv.offsetLeft || pageVerticalPosition + .5 * containerDiv.offsetWidth > about3ContainerDiv.offsetLeft + about3ContainerDiv.offsetWidth ? clearInterval(blinkNbaPlayerTimer) : ($(nbaPlayerEyesDiv).fadeTo(0, 1),
...
function disableIsSeaAnimalStillAnimating(e) {
    var t = e;
    t == fishArray && (fishAnimateNumber >= t.length - 1 ? (isFishStillAnimating = !1,
    fishAnimateNumber = 0) : fishAnimateNumber += 1),
    t == crabArray && (crabAnimateNumber >= t.length - 1 ? (isCrabStillAnimating = !1,
    crabAnimateNumber = 0) : crabAnimateNumber += 1),
    t == turtleArray && (turtleAnimateNumber >= t.length - 1 ? (isTurtleStillAnimating = !1,
    turtleAnimateNumber = 0) : turtleAnimateNumber += 1)
}
function animateRobbyRunSwim() {
    1 == canAnimateRobbyRunSwim && 0 == isRobbyJumping && 0 == isRobbyFalling && "vertical" != layersMovement && (disableAnimateRobbyRunSwim(),
...
function shiftRobbyFrame() {
    ...
    if (1 == isRobbySwimming && 1 == isRobbyBelowSeaLevel ? (robbyStartFrame = robbyStartSwimFrame,
    robbyStopFrame = robbyStopSwimFrame) : (robbyStartFrame = robbyStartRunFrame,
    ...
    robbyStartFrame + counter + switcher == robbyStartFrame && (pageVerticalPositionWhenAnimateRobby1 = pageVerticalPosition),
...
function positionChainBlockAndStringContainer() {
    for (var e = 0; e < chainBlockAndStringContainerArray.length; e++)
        ...
        chainBlockAndStringContainerArray[e].style.bottom = 1 == canAnimateBossInformation ? .8 * containerDiv.offsetHeight + experienceTextContainerArray[e].offsetHeight + "px" : experienceTextContainerDistanceFromFloor + experienceTextContainerArray[e].offsetHeight + "px"
}
...
function positionExperience1Elements() {
    ...
    "internet explorer" == browserName && browserVersion <= 8 || $(piechartRobotFrontDiv).fadeTo(0, 0)
}
function positionExperience2Elements() {
    ...
    "internet explorer" == browserName && browserVersion <= 8 || $(piechartSquidFrontDiv).fadeTo(0, 0)
}
function positionExperience3Elements() {
    ...
    "internet explorer" == browserName && browserVersion <= 8 || $(piechartAlienFrontDiv).fadeTo(0, 0)
}
    ...
    if ("horizontal" == layersMovement) {
        if (0 == isRobbySwimming)
            for (var e = 0; e < landInformationContainerArray.length; e++)
                ...
                landInformationContainerArray[e] == about2ContainerDiv && (animateBuildingsEyes(),
                1 == canAnimateBuildingInformation && (animateBuildings(),
                canAnimateBuildingInformation = !1)),
                landInformationContainerArray[e] == about3ContainerDiv && (animateNbaPlayerEyes(),
                1 == canAnimateNbaInformation && (animateNbaPlayer(),
                canAnimateNbaInformation = !1)),
...
function openAndCloseSquidHands() {
    8 <= openAndCloseSquidHandsCounter ? (openAndCloseSquidHandsCounter = 0,
    clearInterval(moveSquidHandsTimer),
    openSquidHands(),
...
function rotateAlienHands() {
    (alienSteerPreviousAngle = alienSteerAngle) < (alienSteerAngle += alienSteerAngleIncrement) ? alienSteerAngleLimit < alienSteerAngle && (alienSteerAngleIncrement *= -1,
    alienSteerAngleLimit *= -1) : alienSteerAngle < alienSteerAngleLimit && (alienSteerAngleIncrement *= -1,
    alienSteerAngleLimit *= -1),
    0 == alienSteerAngle && (pageVerticalPosition + .5 * containerDiv.offsetWidth < experience3ContainerDiv.offsetLeft || pageVerticalPosition + .5 * containerDiv.offsetWidth > experience3ContainerDiv.offsetLeft + experience3ContainerDiv.offsetWidth) ? (clearInterval(animateAlienHandsTimer),
    ...
    alienSteerDiv.style.transform = "rotate(0deg)") : (alienSteerDiv.style.webkitTransform = "rotate(" + alienSteerAngle + "deg)",
...
function animatePiechartAolTextCode() {
    ...
    $(piechartRobotTextCode2Div).stop().animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartAolTextGraphic() {
    ...
    $(piechartRobotTextGraphic2Div).stop().delay(300).animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartAolTextAnimation() {
    ...
    $(piechartRobotTextAnimation2Div).stop().delay(600).animate({
        opacity: 1
    }, 1e3, function() {})
}
...
function animatePiechartIncognitoTextCode() {
    ...
    $(piechartSquidTextCode2Div).stop().animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartIncognitoTextAnimation() {
    ...
    $(piechartSquidTextAnimation2Div).stop().delay(300).animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartIncognitoTextGraphic() {
    ...
    $(piechartSquidTextGraphic2Div).stop().delay(600).animate({
        opacity: 1
    }, 1e3, function() {})
}
...
function animatePiechartFoxnewsTextCode() {
    ...
    $(piechartAlienTextCode2Div).stop().animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartFoxnewsTextAnimation() {
    ...
    $(piechartAlienTextAnimation2Div).stop().delay(300).animate({
        opacity: 1
    }, 1e3, function() {})
}
function animatePiechartFoxnewsTextGraphic() {
    ...
    $(piechartAlienTextGraphic2Div).stop().delay(600).animate({
        opacity: 1
    }, 1e3, function() {})
}
...
function happyRobby() {
    0 == isRobbyHappy && (clearInterval(happyRobbyTimer),
...
function clearHappyRobbyTimer() {
    1 == isRobbyHappy && (clearInterval(happyRobbyTimer),
    isRobbyHappy = !1)
}
...
function positionRobbyContainerVertically() {
    1 == isPreloadShiftUpAnimationFinish && ($(robbyContainerDiv).stop(!0, !1),
    setRobbyStaticFrame(),
    1 == isRobbySwimming ? positionRobbyAtSeaFloorLevel() : (checkElevationNumberBelowRobby(),
    null != elevationNumberBelowRobby ? robbyContainerDiv.style.bottom = containerDiv.offsetHeight - elevationArray[elevationNumberBelowRobby].offsetTop + "px" : positionRobbyAtGroundLevel()))
}
...
function animateScrollOrSwipeTextContainer() {
    1 == canAnimateScrollOrSwipeTextContainer && (canAnimateScrollOrSwipeTextContainer = !1,
...
function turnOnAndOffScrollOrSwipeTextContainer() {
    "computer" == deviceName ? ($(scrollOrSwipeTextContainer1Div).fadeTo(0, 1),
    $(scrollOrSwipeTextContainer1Div).stop().delay(500).animate({
        opacity: 0
    }, 0, function() {})) : ($(scrollOrSwipeTextContainer2Div).fadeTo(0, 1),
...
function positionContactConfirmationContainer() {
    for (var e = 0; e < contactConfirmationContainerArray.length; e++)
        contactConfirmationContainerArray[e].style.left = "not moving 1" == layersMovement || "not moving 2" == layersMovement ? robbyContainerDiv.offsetLeft + "px" : robbyMaxHorizontalDistance + "px",
...
function drawFirework() {
    drawFireworkCounter >= fireworkArray.length ? (drawFireworkCounter = 0,
    clearInterval(drawFireworkTimer)) : (clearInterval(drawOneLayerOfFireworkTimer),
...
var blinkRobbyEyesTimer, contentDiv = document.getElementById("content"), pageDiv = document.getElementById("page"), robbyContainerDiv = document.getElementById("robby-container"), robbyDiv = document.getElementById("robby"), robbyFramesDiv = document.getElementById("robby-slides"), robbyEyesCloseDiv = document.getElementById("robby-eyes-close"), bannersContainerDiv = document.getElementById("banners-container"), isPreloadShiftUpAnimationFinish = !1, canFinishShiftUpHorizontalLayersAfterEverythingLoaded = !0, splashContainerDiv = document.getElementById("splash-container"), balloonDiv = document.getElementById("balloon"), groundAndGrassContainer1Div = document.getElementById("ground-and-grass-container-1"), elevation1Div = document.getElementById("elevation-1"), elevation2Div = document.getElementById("elevation-2"), layerHorizontalArray = new Array, layerVerticalArray = new Array, gapBetweenContactCloudAndBannersContainer = 400, layerHorizontalSpeedArray = new Array, layerVerticalSpeedArray = new Array, sea1Div = document.getElementById("sea-1"), seaFloorDiv = document.getElementById("sea-floor"), seaFloorFrontObjectArray = new Array, seaFloorBackObjectArray = new Array, about1ContainerDiv = document.getElementById("plants-container"), plantLine1Div = document.getElementById("plant-line-1"), plantLine2Div = document.getElementById("plant-line-2"), plantArray = new Array, plantTargetTopObjectArray = new Array;
...
var canAnimateCrabInformation, skill2ContainerDiv = document.getElementById("skill-2-container"), crabArray = new Array, crabEyeArray = new Array, isCrabStillAnimating = !1, crabAnimateNumber = 0, numberOfCrabInEachRowArray = new Array;
numberOfCrabInEachRowArray.push(5, 5, 4, 3);
var canAnimateTurtleInformation, skill3ContainerDiv = document.getElementById("skill-3-container"), turtleArray = new Array, turtleEyeArray = new Array, isTurtleStillAnimating = !1, turtleAnimateNumber = 0, numberOfTurtleInEachRowArray = new Array;
numberOfTurtleInEachRowArray.push(4, 2, 3, 2);
var isRobbyJumping, isRobbyFalling, swimUpHeight, layersMovement, robbyRightEdge, robbyLeftEdge, robbyMaxHorizontalDistance, canAnimateRobbyRunSwim, robbyStartFrame, robbyStopFrame, shiftRobbyFrameTimer, pageVerticalPositionWhenAnimateRobby1, pageVerticalPositionWhenAnimateRobby2, canAnimateNbaInformation, nbaBoardsCounter, nbaBoardsAnimationTimer, nbaPlayerCounter, nbaPlayerTimer, blinkNbaPlayerTimer, canAnimateSocialContainer, happyRobbyTimer, scrollOrSwipeTextContainerTimer, waterfallTimer, drawFireworkTimer, fireworkCenterX, fireworkCenterY, fireworkOneRadiusDistance, fireworkOneRotationAngle, drawOneLayerOfFireworkTimer, pageVerticalPosition = 0, pageVerticalPositionOnTouch = 0, previousPageVerticalPosition = 0, deltaPageVerticalPosition = 0, isRobbySwimming = !1, isRobbyBelowSeaLevel = !1, elevationArray = new Array, elevationNumberBelowRobby = null, distanceBetweenRobbyAndBalloon = 150, counter = 0, switcher = 1, robbyStaticFrame = 0, robbyStartRunFrame = 1, robbyStopRunFrame = 2, robbyStartSwimFrame = 3, robbyStopSwimFrame = 4, robbySwimDownFrame = 5, robbyStartJumpFrame = 6, robbyStopJumpFrame = 7, robbyOneFrameWidth = 200, shiftRobbyFrameTimeInterval = 200, minimumVerticalDistanceToTriggerRobbySwimDownFrame = 100, nbaBoardArray = new Array, about3ContainerDiv = document.getElementById("nba-container"), nbaPlayerDiv = document.getElementById("nba-player"), nbaPlayerContainerDiv = document.getElementById("nba-player-container"), nbaPlayerFrameDiv = document.getElementById("nba-player-frame"), nbaRimContainerDiv = document.getElementById("nba-rim-container"), nbaBallDiv = document.getElementById("nba-ball"), nbaPlayerEyesDiv = document.getElementById("nba-player-eyes"), contactContainerDiv = document.getElementById("contact-container"), socialContainerDiv = document.getElementById("social-container"), isRobbyHappy = !1, scrollOrSwipeTextContainer1Div = document.getElementById("scroll-or-swipe-text-container-1"), scrollOrSwipeTextContainer2Div = document.getElementById("scroll-or-swipe-text-container-2"), canHideScrollOrSwipeTextContainer = !0, canAnimateScrollOrSwipeTextContainer = !0, contactConfirmationContainerArray = new Array, emailAddressDiv = document.getElementById("email-address"), emailSubjectDiv = document.getElementById("email-subject"), emailMessageDiv = document.getElementById("email-message"), isContactConfirmationContainerVisible = !0, waterfall1Div = document.getElementById("waterfall-1"), waterfall2Div = document.getElementById("waterfall-2"), touchStartX = 0, touchCurrentX = 0, touchEndX = 0, fireworksContainerDiv = document.getElementById("fireworks-container"), fireworkArray = new Array, fireworkSvgArray = new Array, cArray = new Array, drawFireworkCounter = 0, fireworkRowNumber = 8, fireworkColumnNumber = 16, fireworkLayerNumber = 0, fireworkDotRadius = 5, canDrawManyFireworks = !0;
disableIsRobbyJumpingAndFalling();
